...
async def list_device_models(db: DBSession, pg: PaginationDep, platform_id: uuid.UUID | None = None):
    q = select(DeviceModel).options(selectinload(DeviceModel.platform))
    if platform_id:
...
async def get_device_model(model_id: uuid.UUID, db: DBSession):
    result = await db.execute(
        select(DeviceModel).options(selectinload(DeviceModel.platform)).where(DeviceModel.id == model_id)
    )
